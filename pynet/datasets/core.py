...
class DataManager(object):
    ...
    def __init__(self, input_path, metadata_path, output_path=None,
                 ...
                 weighted_random_sampler=False, input_transforms=None,
                 output_transforms=None, labels_transforms=None,
                 stratify_label_transforms=None, data_augmentation=None,
                 add_input=False, patch_size=None, input_size=None,
        ...
        custom_stratification: dict, default None
            same format as projection labels. It will split the dataset into
            train/test/val according to the stratification defined in the dict.
        ...
        weighted_random_sampler: bool, default False
            Whether we use a weighted random sampler (to deal with imbalanced
            classes issue) or not
        ...
        if stratify_label is not None:
            ...
            # Apply the labels transform here as a mapping to the integer
        ...
        if self.weighted_random_sampler:
            if self.stratify_label is None:
                raise ValueError("Impossible to use the WeightedRandomSampler "
                                 "if no stratify label is available.")
            class_samples_count = [
                0 for _ in range(len(set(self.stratify_label[mask])))]
            for label in self.stratify_label[mask]:
                class_samples_count[label] += 1
            # Imbalanced weights in case of imbalanced classes issue
            self.sampler_weigths = (
                1. / torch.tensor(class_samples_count, dtype=torch.float))
        ...
        if custom_stratification is not None:
            if ("validation" in custom_stratification and
                    stratify_label is not None):
                print("Warning: impossible to stratify the data: "
                      "validation+test set already defined ! ")
            train_mask, test_mask = (
                DataManager.get_mask(df, custom_stratification["train"]),
            ...
            if test_size == 1:
                ...
                splitter = ShuffleSplit(
                    n_splits=1, random_state=0, test_size=test_size)
                train_indices, test_indices = next(
                    splitter.split(dummy_like_X_masked))
                train_indices = mask_indices[train_indices]
                test_indices = mask_indices[test_indices]
        ...
        if val_indices is not None:
            ...
            gen = [(train_indices[tr], train_indices[val])
                   for (tr, val) in gen]
        ...
        for fold_train_index, fold_val_index in gen:
            ...
            assert (len(set(fold_val_index)) + len(set(fold_train_index)) ==
        ...
        for key in ("inputs", "outputs", "labels"):
            if (len(list_samples) == 0 or
                ...
                data[key] = torch.stack([
                    torch.as_tensor(getattr(s, key))
                    for s in list_samples], dim=0).float()
    ...
    def get_dataloader(self, train=False, validation=False, test=False,
        ...
        if train:
            if self.weighted_random_sampler:
                ...
                samples_weigths = self.sampler_weigths[
                    np.array(self.stratify_label[indices], dtype=np.int32)]
...
class ArrayDataset(Dataset):
    ...
    def __init__(self, inputs, indices, labels=None, outputs=None,
                 ...
                 output_transforms=None, label_transforms=None,
                 patch_size=None, input_size=None):
        ...
        if self.patch_size is not None:
            assert (np.array(self.patch_size).shape ==
                    np.array(self.input_size).shape)
            self.nb_patches_by_img = (
                np.product(np.array(self.input_size) //
                           np.array(self.patch_size)))
            (self.input_cached, self.output_cached, self.label_cached,
             self.input_indx_cached) = None, None, None, None
    ...
    def __getitem__(self, item):
        ...
        if self.patch_size is not None:
            ...
            if self.input_indx_cached == item:
                ...
                indx = np.unravel_index(
                    offset,
                    np.array(self.input_size) // np.array(self.patch_size))
        ...
        # Eventually, get only one patch of the input (and one patch of the
        # corresponding output if add_input==True)
        if self.patch_size is not None:
            self.input_indx_cached = item
            from skimage.util.shape import view_as_blocks
            # from a flat index, convert it to an nd-array index
            indx = np.unravel_index(
                offset, np.array(self.input_size) // np.array(self.patch_size))
            ...
            if self.output_same_as_input:
                self.output_cached = view_as_blocks(
                    _outputs, tuple(self.patch_size))
