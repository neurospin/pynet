...
def fetch_surface(hemisphere):
    ...
    def fetch_surface_hemi(datasetdir=SAVING_FOLDER,
        ...
        data = pd.read_table(FILES["clinical_genetic_surface"]).drop(columns=["bloc-t1w_hemi-lh_metric-area", "bloc-t1w_hemi-rh_metric-area"])
        ...
        if test_size != 0:
            path_test = os.path.join(datasetdir, "EUAIMS_surface_{}_X_test.npy".format(hemisphere))
